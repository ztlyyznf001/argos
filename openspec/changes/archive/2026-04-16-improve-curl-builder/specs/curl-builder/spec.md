...
系统 SHALL 按照以下顺序生成 cURL 命令：`-i` 标志 → HTTP 方法（可选）→ 请求头 → `--compressed`（可选）→ 请求体（可选）→ URL。

#### Scenario: 标准命令结构
- **WHEN** 调用 `KurilCurlBuilder.build(record)` 且请求方法为 POST
- **THEN** 生成命令以 `curl -i` 开头，依次包含 `-X POST`、各 `-H` 参数，最后为单引号包裹的 URL

#### Scenario: URL 位于末尾
- **WHEN** 调用 `KurilCurlBuilder.build(record)`
- **THEN** URL 出现在命令的最后一个参数位置，格式为 `'<url>'`
...
- **WHEN** `record.method` 为 `POST`、`PUT`、`DELETE`、`PATCH` 等
...
### Requirement: -i 标志
系统 SHALL 在所有生成的 cURL 命令中包含 `-i` 标志，使运行时同时输出响应头。

#### Scenario: 命令始终包含 -i
- **WHEN** 调用 `KurilCurlBuilder.build(record)` 对任意请求
- **THEN** 生成命令中包含 `-i` 标志
