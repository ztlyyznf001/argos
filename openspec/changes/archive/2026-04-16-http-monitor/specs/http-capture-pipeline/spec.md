...
### Requirement: HttpOverrides 正确包装为 KurilHttpClient
系统 SHALL 在 `KurilHttpOverrides.createHttpClient()` 中将底层 `HttpClient` 包装为 `KurilHttpClient`，使所有 HTTP 请求都经过监控拦截链路。

#### Scenario: 所有 HTTP 请求被拦截
- **WHEN** 应用发起任意 HTTP/HTTPS 请求（GET、POST、PUT、DELETE 等）
- **THEN** 请求经过 `KurilHttpClient` → `KurilHttpClientRequest` → `KurilHttpClientResponse` 完整链路，请求头、请求体、响应码、响应头、响应体均被捕获

#### Scenario: 代理配置在包装后仍生效
- **WHEN** `KurilHttpMonitor.instance.customProxy` 设置了代理地址
- **THEN** 包装后的 `KurilHttpClient` 内部 `origin` 上的 `findProxy` 使用该代理，请求流量路由到指定代理

### Requirement: 请求耗时完整记录
系统 SHALL 在响应接收完成时写入 `endTimestamp`，使 `KurilPacketRecord.durationMs` 返回真实耗时。

#### Scenario: 正常响应记录耗时
- **WHEN** 一次 HTTP 请求成功收到响应
- **THEN** `KurilHttpInfo.response.endTimestamp` 为响应接收时的 Unix 毫秒时间戳，`durationMs = endTimestamp - startTimestamp > 0`
...
系统 SHALL 在请求发生网络错误（连接失败、超时、DNS 解析失败）时将错误信息写入 `KurilHttpInfo.error` 并触发与正常响应相同的存储和 listener 回调。
...
- **THEN** `KurilApmManager.instance.listener` 被调用，参数为携带 `error` 字段的 `KurilHttpInfo` 实例
...
- **WHEN** `KurilHttpClientResponse` 的 `listen()` 与 `transform()` 均被调用（如某些 HTTP 库同时使用两个接口）
