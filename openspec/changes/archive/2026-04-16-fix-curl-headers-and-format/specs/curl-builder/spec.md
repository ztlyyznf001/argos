...
系统 SHALL 在 `KurilCurlBuilder.build()` 中支持可选的 `proxy` 命名参数；若传入非空值，则在命令末尾追加 `--proxy <proxy>`。

#### Scenario: 传入 proxy 时追加 --proxy
- **WHEN** 调用 `KurilCurlBuilder.build(record, proxy: 'http://localhost:9091')`
- **THEN** 生成命令末尾包含 `--proxy http://localhost:9091`

#### Scenario: 不传 proxy 时无 --proxy
- **WHEN** 调用 `KurilCurlBuilder.build(record)`（不传 proxy 参数）
- **THEN** 生成命令中不包含 `--proxy`
