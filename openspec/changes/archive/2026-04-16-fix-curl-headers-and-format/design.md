...
`KurilHttpClientRequest` 用 `recordParameter(data)` 在写 body 时顺带记录 headers：
```dart
...
```
GET 请求不调用 `add()`，所以 `httpInfo.request.header` 始终为 `null`，`KurilHttpInfo.toJson()` 里 `headersToMap(null)` 返回空 Map，curl 命令生成时无任何 `-H` 参数。
...
```dart
static String build(KurilPacketRecord record, {String? proxy})
```
