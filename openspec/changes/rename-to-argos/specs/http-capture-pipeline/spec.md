...
- **THEN** 该记录以 `responseCode = 0`、`responseBody = ''`、`error` 字段非空的形式存入存储后端
...
- **THEN** 存储后端中该请求只存在一条记录，listener 只被回调一次
